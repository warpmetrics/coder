 ...
 */
export function inferDependencies(sessionId, repos, workdir, { model = 'sonnet', log } = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      output = execFileSync('claude', [
        ...
        '--output-format', 'json',
      ], { encoding: 'utf-8', cwd: workdir, timeout: 60000, stdio: ['pipe', 'pipe', 'pipe'] });
    } catch (err) {
 ...
 * @returns {{ release: Array<{repo, command, dependsOn}> }} or null
 */
export function inferDeployPlan(sessionId, prs, deployConfig, workdir, { model = 'sonnet', log } = {}) {
  if (!prs?.length) {
  ...
  try {
    dependencies = inferDependencies(sessionId, repos, workdir, { model, log });
  } catch (err) {
