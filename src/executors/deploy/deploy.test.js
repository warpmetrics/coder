...
describe('deploy executor', () => {
  ...
  it('single step, success', async () => {
    ...
    assert.equal(gitMock._cloneCalls.length, 1);
    assert.ok(gitMock._cloneCalls[0].url.includes('org/api'));
  ...
  it('multiple steps in DAG order — clones once per repo, runs in level order', async () => {
    ...
    // Clone once per repo
    assert.equal(gitMock._cloneCalls.length, 3);
  ...
  it('repos with same basename get unique directories', async () => {
    ...
    assert.equal(result.type, 'success');
    assert.equal(gitMock._cloneCalls.length, 2);

    // Destinations must be different (deriveRepoDirNames handles collision)
    const dests = gitMock._cloneCalls.map(c => c.dest);
