...
import { buildTrace, fetchComments } from '../src/executors/claude/index.js';
...
describe('fetchComments', () => {
  ...
  it('returns empty when no comments', () => {
    const codehost = { getIssueComments: () => [] };
    const result = fetchComments(codehost, 42, 'owner/repo');
    assert.equal(result.commentsText, '');
    assert.equal(result.lastHumanMessage, null);
  });

  it('returns empty on error', () => {
    const codehost = { getIssueComments: () => { throw new Error('API error'); } };
    const result = fetchComments(codehost, 42, 'owner/repo');
    assert.equal(result.commentsText, '');
    assert.equal(result.lastHumanMessage, null);
  });
