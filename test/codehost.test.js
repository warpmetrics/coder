...
import { merge } from '../src/executors/codehost.js';
...
function makeCodehost(overrides = {}) {
  ...
  return {
    ...
    botComment: overrides.botComment || (() => {}),
    _calls: calls,
  };
}
...
function makeContext(overrides = {}) {
  ...
  return {
    ...
    codehost: makeCodehost(overrides.codehostOverrides),
  ...
  it('post-merge summary comment failure is non-fatal', async () => {
    ...
    const ctx = makeContext({
      codehost: makeCodehost({
        ...
        botComment: () => { throw new Error('comment API down'); },
      }),
    });
    const result = await merge(makeItem(), ctx);
